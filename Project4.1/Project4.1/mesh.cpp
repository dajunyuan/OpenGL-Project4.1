...
void My_Mesh::generate_disk(int num_division)
{
	...
	float rr = pi / 180;  //角度转弧度常数
						  //圆柱体Z轴向上，按cos和sin生成x，y坐标
	for (int i = 0; i <1; i++)
	{
		float x =0;
		float y =0;
		m_vertices_.push_back(x);  //存入顶点
	...
	for (int i = 1; i < num_samples+1; i++)
	{
		...
		int a=(i ) % num_samples;
		int b = (i +1) % num_samples;
		m_faces_.push_back(a);
		m_faces_.push_back(b);
		...
		m_vt_list_.push_back(0.5);
		//纹理坐标
		m_vt_list_.push_back(m_vertices_[3*a]*0.5+0.5);
		m_vt_list_.push_back(m_vertices_[3 * a+1] * 0.5 + 0.5);
		//纹理坐标
		m_vt_list_.push_back(m_vertices_[3 * b] * 0.5 + 0.5);
		m_vt_list_.push_back(m_vertices_[3 * b+1] * 0.5 + 0.5);
		//纹理坐标
